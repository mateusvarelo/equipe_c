...
class CondorWebsite():
    ...
    def _build_url_search(self, search: str):
        ...
        log.debug(f"Pesquisa tratada {search_converter} e a original {search}")
        url = f'{self.base_url}/{search_converter}'
        self.build_url_search = url
        log.debug(f"Url {self.build_url_search}")
        return url
    ...
    def add_metadata(self, 
                     d: dict, 
                     created_at:datetime =datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f%z')) -> dict:
